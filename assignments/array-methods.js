...
let allCaps = runners.map((caps) => {
  return {'first_name': caps.first_name.toUpperCase()};
});
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((total, donos) => {
  return total += donos.donation;
}, 0);
...
// Problem 3
let donoTotal = runners.filter((donos) => {
  return donos.donation > 100;
});
console.log(donoTotal);